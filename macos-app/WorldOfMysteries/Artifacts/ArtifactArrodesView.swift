...
public struct ArrodesMirrorArtifactView: View {
      ...
      VStack(alignment: .leading, spacing: DesignTokens.LayoutInsets.stackSpacingLg) {
        ...
        .overlay(
          RoundedRectangle(cornerRadius: DesignTokens.Radii.md).stroke(
            Color.Mystic.spiritualBlue.opacity(0.22), lineWidth: DesignTokens.Borders.hairline))

        ArtifactSection(
          "提出问题", caption: "Question → Context Compiler → Reveal Policy → Answer", tone: .azure
        ) {
          HStack(spacing: DesignTokens.Spacing.xs) {
            ForEach(["人物", "事件", "地点", "物品"], id: \.self) { item in
              Button {
                ...
                MysticBadge(
                  item, tone: focus == item ? .azure : .neutral, systemIcon: focusIcon(item),
          ...
          ArtifactSection("对等原则", caption: "回答或拒绝都形成后果", tone: .amber) {
            if let prompt = model.lastResolution?.followUpPrompt {
              ArtifactTypewriterText(
                "“\(prompt)”", font: Font.Mystic.narrativeSubtitle,
            ...
            HStack {
              Button("拒绝回答") {
                model.performDetached(
                  .init(
                    artifactID: .arrodesMirror, action: .refuseExchange, context: context,
                    input: "refused"))
              }
              ...
              ArtifactHoldToCommitButton(
                "完成交换", systemImage: "arrow.left.arrow.right", tone: .azure,
                ...
                Task {
                  await model.perform(
                    .init(
                      artifactID: .arrodesMirror, action: .answerExchange, context: context,
                      input: answer))
                  if model.lastResolution?.state == "exchange_complete" { exchangeAnswer = "" }
                }
              }
            }
          }
  ...
  private var exchangeIsDue: Bool { model.lastResolution?.state == "exchange_due" }
  ...
  private func ask() {
    ...
    model.performDetached(
      .init(
        artifactID: .arrodesMirror, action: .ask, context: context, input: body,
