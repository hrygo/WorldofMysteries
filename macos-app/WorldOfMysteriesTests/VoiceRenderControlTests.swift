...
struct VoiceRenderControlTests {
    @Test("Request decodes the committed speech and revision binding exactly")
    func requestRoundTrip() throws {
        ...
        let roundTrip = try JSONDecoder().decode(
            VoiceRenderControlRequestDTO.self,
            from: JSONEncoder().encode(decoded)
        )
        #expect(roundTrip == decoded)
    }

    @Test("Accepted response keeps render and media identities correlated")
