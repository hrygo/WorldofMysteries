...
### 7.3 L3: 统一门禁执行器 (`scripts/gate_runner.sh`)
将 `Go_NoGo_Gates_v1.0.yaml` 中定义的 5 大 P0 门禁固化为一键测试脚本：

```bash
./scripts/gate_runner.sh --all-p0
```
...
1. **落成架构适应度脚本**：编写 [`scripts/check_architecture_fitness.py`](file:///Users/hrygo/Documents/WorldofMysteries/scripts/check_architecture_fitness.py)，纳为每次 commit/push 的必跑前置检查；
2. **完善统一门禁脚本**：编写 [`scripts/gate_runner.sh`](file:///Users/hrygo/Documents/WorldofMysteries/scripts/gate_runner.sh)，串联 Swift Testing、pytest、Schema 验证与一致性检查；
