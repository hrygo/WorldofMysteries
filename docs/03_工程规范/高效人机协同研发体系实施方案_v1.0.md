...
> ⚠️ **已被取代（2026-09-16）**：本 v1.0 文档描述的 HACF 2.0 机制（共享 `.venv` 软链、胶囊携带
> `verification_commands`、向胶囊回写 `attestation`、`forbidden_patterns` 无判定点、PR 卡片硬编码通过状态）
> 已被 [`高效人机协同研发体系实施方案_v1.1.md`](./高效人机协同研发体系实施方案_v1.1.md) 取代。
> 本文件仅作历史记录保留，**请勿据其实施**。决策依据见
> [`ADR-004`](../01_总体架构/ADR-004_协同层门禁主权与凭证分离_v1.0.md)。
