...
[Persistent World Alpha 推进与验收计划](06_实施基线/Persistent_World_Alpha_Plan_v1.0.md) 已记录用户确认的近期方向：真实一轮 → 五轮与重启连续性 → 真实模型与基础语音 → 独立使用体验。下一步为 M1 真实 IPC，与数据内核及 Golden 测试准备并行；不再指向已完成的 UI 设计。

[项目状态](PROJECT_STATE.json) 区分历史报告与当前未验证能力。此处更新的是执行优先级，**不代表真实 Engine、五轮回归、macOS 验收或正式美术交付已完成**；既有门禁继续适用。
