...
`6acb2fd` 的实际 Release 日志明确返回 `socket-path-budget; code 107`：
沙箱临时目录加随机私有目录与 `engine.sock` 后超过 Darwin 104 字节上限。
独立探针的较短容器标识没有触发该边界，故不能用其成功代替实际 App 验收。

端点名改为内部短名 `s`，按实际 UTF-8 字节计算目录预算，而不是估算固定后缀。
保留原 0700 私有随机目录、启动锁、鉴权与 inode 清理；不改工作目录、不创建指向包外的别名，
不增加共享全局临时目录。明确指定且过长的测试根仍拒绝，不偷偷改变调用者根目录。
新增真实文件系统回归覆盖原 107 字节边界、权限与清理，另覆盖不可容纳路径和多字节路径。
这些端点每次启动重新分配，不是存档标识，没有用户数据迁移。

## 构建阶段下载可靠性

`674755d` 首次打包运行的 Release 编译成功，随后固定归档下载遇到 HTTP 504。
下载器现在仅对明确的暂时性 HTTP/连接中断最多尝试三次，使用 1/2 秒退避；每次都从同一
锁定 URL 重新读取到独立临时文件，绝不切换版本或来源。完整下载后仍强制校验字节数与
SHA-256，再以原子、不可覆盖的方式发布到归档路径。失败或中断不留下可被后续步骤误用的残包。
认证/授权错误、TLS 验证失败、超长响应、哈希失配及不安全重定向立即失败；已有文件、符号
链接和并发构建产生的目标均不覆盖。该重试只作用于构建机，不新增最终用户下载或等待步骤。

本地更新后的归档与打包专项共 **79 项通过**，包括 HTTP 504 后成功、部分响应断线、重试耗尽、
校验失败、旧文件/链接保护及并发发布竞争。目标平台验收仍以精确候选的实际打包结果为准。


## Release 进程身份观测

同一 `674755d` 的第二次打包执行已完成依赖、签名、三次探针启动与崩溃恢复，
但实际 GUI 的子进程识别仍超时。观测代码把 `ps command` 的显示文本当作文件路径；
Darwin 的该输出经过 `strvis`，不能作为中文安装目录的稳定路径身份。
现只用 `ps` 读取数字父子关系，使用内核 `proc_pidpath` 读取未转义可执行文件，再用
文件身份比对包内解释器，拒绝同名但不同文件、身份读取失败和重复引擎。该 API 仅在构建机
验证脚本中使用，不进入发行 App，不改变沙箱权限、生产逻辑或验收成功条件。
该修正的最终有效性仍须实际 Release 搬迁、引擎恢复及父进程退出测试共同证明。
