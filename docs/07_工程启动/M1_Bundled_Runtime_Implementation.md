...
`79c9d99` 的锁定依赖导入与离线消息探针已成功；原生检查随后误将 Tcl 库自己的
`LC_ID_DYLIB` 当作加载依赖。现以 `otool -D` 的实际身份对应 `-L` 首行，只移除该身份项；
其余真实依赖仍执行完整路径限制，身份不一致直接失败。新增正反例覆盖该边界。

## macOS 回归增量 3

`6892038` 完成原生架构/依赖检查与 App 签名校验后，实际 entitlements 回读发现解释器
没有继承权限。根因是同一文件的 `/var` 与 `/private/var` 路径别名不相等，签名时没有命中
解释器分支。修复为解析后文件身份比较，并规范化构建工作目录；新增父目录符号链接回归。
不放宽权限校验、不添加例外权限，仍需后续目标平台重跑确认。


## macOS GUI 启动诊断

`d3a1f31` 的签名沙箱探针已成功导入包内依赖、完成三次实际启动/握手与崩溃恢复，
启动到 health 的三个样本为 738.16 / 716.12 / 672.74 ms。实际 Release GUI 正常显示，
但提示运行环境无效，进程现场确认没有启动子引擎。因此并非仅凭进程表格式猜测失败原因。
新增固定阶段标识与数值诊断，定位私有运行目录和 socket 预算拒绝；日志不含路径、凭据或载荷。
新增诊断不改变用户错误文案、权限、目录策略或超时；打包验收仍保持失败直至实际问题修复。
