...
def test_runtime_stager_packages_module_relative_story_content():
    source = (Path(__file__).resolve().parents[2]/'scripts/bundle_engine.py').read_text()
    # The trusted artifact is generated by the same build script the repository
    # tests exercise, then probed from a foreign cwd so a repository-relative
    # lookup can never pass packaging.
    assert 'build_story_content.py' in source
    assert "'story_content': story_content_summary" in source
    assert 'Packaged story content is not module-relative' in source
