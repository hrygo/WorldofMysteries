"""Safe ordered world.db v1→v10 migration and Story bootstrap schema tests."""
...
def test_fresh_world_initializes_directly_to_v10_without_migration_backup(tmp_path):
    ...
    with closing(connect(path)) as conn:
        ...
        integrity(conn)
    assert {"story_sessions", "story_session_bootstraps", "story_state_deltas", "turn_transactions", "turn_intake_commands", "turn_advice_interpretations", "voice_bindings", "narrative_blocks", "delivery_cursors", "audio_takes", "audio_tracks", "audio_track_units"} <= _tables(path)
...
def test_existing_v1_world_is_backed_up_then_migrated_without_data_loss(tmp_path):
    ...
    backup = path.with_name("world.db.pre-migration-v1-to-v10.bak")
...
def test_existing_v3_world_gets_narrative_table_with_recoverable_backup(tmp_path):
    ...
    backup = path.with_name("world.db.pre-migration-v3-to-v10.bak")
...
def test_existing_v4_world_gets_delivery_cursor_table_with_recoverable_backup(tmp_path):
    ...
    backup = path.with_name("world.db.pre-migration-v4-to-v10.bak")
...
def test_existing_v5_world_gets_audio_take_table_with_recoverable_backup(tmp_path):
    ...
    backup = path.with_name("world.db.pre-migration-v5-to-v10.bak")
...
def _build_v6_world(path: Path, *, store_id: str = "store-v6") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with closing(connect(path)) as conn:
        conn.execute("BEGIN IMMEDIATE")
        directory = Path(database_schema.__file__).with_name("migrations")
...
def test_existing_v6_world_gets_storybook_track_tables_with_recoverable_backup(tmp_path):
    path = tmp_path / "Worlds" / "v6-existing" / "world.db"
    _build_v6_world(path)
    ...
    backup = path.with_name("world.db.pre-migration-v6-to-v10.bak")
...
def _build_v7_world(path: Path, *, store_id: str = "store-v7") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with closing(connect(path)) as conn:
        conn.execute("BEGIN IMMEDIATE")
        directory = Path(database_schema.__file__).with_name("migrations")
...
def test_existing_v7_world_gets_turn_intake_table_with_recoverable_backup(tmp_path):
    path = tmp_path / "Worlds" / "v7-existing" / "world.db"
    _build_v7_world(path)
    ...
    backup = path.with_name("world.db.pre-migration-v7-to-v10.bak")
...
def _build_v8_world(path: Path, *, store_id: str = "store-v8") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with closing(connect(path)) as conn:
        conn.execute("BEGIN IMMEDIATE")
        directory = Path(database_schema.__file__).with_name("migrations")
...
def test_existing_v8_world_gets_player_advice_table_with_recoverable_backup(tmp_path):
    path = tmp_path / "Worlds" / "v8-existing" / "world.db"
    _build_v8_world(path)
    ...
    backup = path.with_name("world.db.pre-migration-v8-to-v10.bak")
